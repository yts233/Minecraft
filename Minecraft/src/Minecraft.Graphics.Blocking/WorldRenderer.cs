...
namespace Minecraft.Graphics.Blocking
{
    public class WorldRenderer : ICompletedRenderer
    {
        ...
        public WorldRenderer(IWorld world, Func<ITextureAtlas> atlasProvider, IMatrixProvider<Matrix4, Vector4> viewMatrix, IMatrixProvider<Matrix4, Vector4> projectionMatrix)
        {
            _world = world;
                ...
                for (var z = minRenderZ; z <= maxRenderZ; z++)
                {
                    for (var x = minRenderX; x <= maxRenderX; x++)
                    {
                        if (!_renderers.TryGetValue((x, z), out ChunkRenderer renderer))
                        {
                            ...
                            if (chunk == null)
                                continue;
                            renderer = new ChunkRenderer(chunk, () => _atlas, _viewMatrix, _projectionMatrix);
