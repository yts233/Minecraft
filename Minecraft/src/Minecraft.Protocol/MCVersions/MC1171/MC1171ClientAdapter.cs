...
namespace Minecraft.Protocol.MCVersions.MC1171
{
    ...
    /// Minecraft 客户端适配器
    /// </summary>
    public class MC1171ClientAdapter : IMinecraftClientAdapter
    {
        ...
        private MCVer756ProtocolAdapter _protocolAdapter;
        private readonly MC1171Client _client;
        private static readonly Logger<MC1171ClientAdapter> _logger = Logger.GetLogger<MC1171ClientAdapter>();

        public MC1171ClientAdapter(string hostname, ushort port, MC1171Client client)
        {
            _hostname = hostname;
            _port = port;
        ...
        public MCVer756ProtocolAdapter GetProtocol()
        {
            if (!IsConnected)
                return null;
        ...
        {
            _protocolAdapter.SendPacket(new ClientChatMessagePacket { Message = message });
        }
        ...
        {
            _protocolAdapter.SendPacket(new PlayerPositionPacket { Position = position, OnGround = onGround });
        }
        ...
        {
            _protocolAdapter.SendPacket(new PlayerMovementPacket { OnGround = onGround });
        }
        ...
        {
            _protocolAdapter.SendPacket(new InteractEntityPacket { EntityId = entityId, Type = InteractType.Attack, Sneaking = sneaking });
        }
        ...
        {
            _protocolAdapter.SendPacket(new InteractEntityPacket { EntityId = entityId, Type = InteractType.Interact, Hand = hand, Sneaking = sneaking });
        }
        ...
        public int ProtocolVersion => throw new NotImplementedException();
        ...
        public void Connect()
        {
            ...
            IsConnected = true;
            _connecting = false;
            _protocolAdapter = new MCVer756ProtocolAdapter(_tcpClient.GetStream(), PacketBoundTo.Server);
            _protocolAdapter.Start();
            Connected?.Invoke(this, (_hostname, _port));

            // C→S: Handshake State=2
            _protocolAdapter.SendPacket(new HandshakePacket
            {
            ...
            // C→S: Login Start
            _protocolAdapter.SendPacket(new LoginStartPacket
            {
        ...
        private void HandlePackets()
        {
            try
            {
                while (IsConnected)
                {
                    var p = _protocolAdapter.ReceivePacket();
                    if (p == null)
                        break;
                    switch (p)
                    {
                        ...
                        case PlayerPositionAndLookPacket packet:
                            _protocolAdapter.SendPacket(new TeleportConfirmPacket { TeleportId = packet.TeleportId });
                            if (!IsJoined)
                            {
                                IsJoined = true;
                                _protocolAdapter.SendPacket(new PlayerPositionAndRotationPacket
                                {
                                    Position = packet.Position,
                                    Rotation = packet.Rotation,
                                    OnGround = true
                                });
                                _protocolAdapter.SendPacket(new ClientStatusPacket
                                {
