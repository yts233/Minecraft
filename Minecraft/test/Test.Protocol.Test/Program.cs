...
namespace Test.Protocol.Test
{
    internal class Program
    {
        private static async Task Main(string[] args)
        {
            ...
            adapter.HandlePacket(delegate(StatusResponsePacket packet) { Console.WriteLine(packet.Content); });
            adapter.HandlePacket(delegate(StatusPongPacket packet)
            {
                Logger.Info<Program>(
            ...
            await adapter
                ...
                .SendPacket(new StatusPingPacket {Payload = now})
                .FlushAsync();
            await adapterTask;
        }
    }
